/*
 * Bitmap manipulation functions. See `libbmp.h` for details.
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#endif
    if(gif)
        return bm_save_gif(b, p_save, p_save_size);
    if(pcx)
...
//     gce.terminator = 0x00;
    ...
    //actual control block intro byte here
    // fputc(0x21, f);
    // fputc(0xF9, f);
    ...
    memcpy(data+0, sig, 3);
    memcpy(data+3, ver, 3);
    memcpy(data+6, &x_size, 2);
    memcpy(data+8, &y_size, 2);
    memcpy(data+10, &gctf, 1);
    memcpy(data+11, &bci, sizeof bci);
    memcpy(data+12, &par, 1);
    memcpy(data+13, &gct, sizeof(*gct)*sgct);

    short gce_offset = 0+13+sizeof(*gct)*sgct;

    //fill out control extension block
    memcpy(data+gce_offset+0, &gce_intro, 1);
    memcpy(data+gce_offset+1, &gce_label, 1);
    memcpy(data+gce_offset+2, &gce_block_size, 1);
    memcpy(data+gce_offset+3, &gce_fields, 1);
    memcpy(data+gce_offset+4, &gce_delay, 2);
    memcpy(data+gce_offset+6, &tci, 1);
    memcpy(data+gce_offset+7, &terminator, 1);

    short img_offset = gce_offset + 8;

    //Fill out image block
    memcpy(data+img_offset+0, &img_intro, 1);
    memcpy(data+img_offset+1, &left, 2);
    memcpy(data+img_offset+3, &top, 2);
    memcpy(data+img_offset+5, &width, 2);
    memcpy(data+img_offset+7, &height, 2);
    memcpy(data+img_offset+9, &im_field, 1);
    memcpy(data+img_offset+10, &lzw_min_code, 1);
    ...
    for(p=0;p<len;p++){
      if(p % 0xFF == 0) {
          /* beginning of a new block; lzw_emit_code the length byte */
          if(len - p >= 0xFF) {
              memcpy(data+bytes_offset+p+newb, &length_byte, sizeof length_byte);
          } else {
              len_p = len-p;
              memcpy(data+bytes_offset+p+newb, &len_p, sizeof len_p);
          }
          newb += 1;
      }
      memcpy(data+bytes_offset+p+newb, &bytes[p], 1);

  }
    int after_bytes = bytes_offset + len + newb;

    memcpy(data+after_bytes+0, &terminator, 1);
    memcpy(data+after_bytes+1, &trailer, 1);
    free(bytes);
    int data_size = after_bytes+2;
    memcpy(p_savegif, data, data_size);
    *p_savegif_size = data_size;
    //
    // FILE *fhack;
    // fhack = fopen("output/gif_hack.gif", "wb");
    // if(fwrite(data, data_size, 1, fhack) != 1) {
    //     fclose(fhack);
    //     SET_ERROR("couldn't write GIF info");
    //     return 0;
    // }
