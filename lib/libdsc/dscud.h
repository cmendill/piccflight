/*
    ...
    RUBY-MM-416
    RUBY-MM-1612
    FP-DAQ1616  // Feature Pak DAQ1616 board.
    FP-GPIO96   // Feature Pak GPIO-96
    OPMM-1616
    VEGA
    ATHENAIII
    Altair
...
#define DSC_VERSION  0x0700  /* 0xVVRR - Version & Revision */
...
#define DSC_FPDAQ1616   37
#define DSC_FPGPIO96    38
#define DSC_OPMM1616    39
#define DSC_HELIOSDV    40
#define DSC_VEGA        41
#define DSC_ATHENAIII   42

#define DSC_ALTAIR      43
#define DSC_RMM1616	44
#define DSC_RMM1616P	45
#define DSC_MPEGPIO	46
#define DSC_MPEDAQ0804  47
#define DSC_EMMDIO4M	48
#define DSC_ARIES       49
#define DSC_DMM2000     50
#define DSC_PLT_T56N_4G 51
#define DSC_HELIX		52
#define DSC_DMM16RPAT   53
#define DSC_DMM16RPAT_PCI   54
#define DSC_ZETA 		55 //added 10/9/2017
...
#define INT_TYPE_COUNTER0   0x11
#define INT_TYPE_COUNTER1   0x12
#define INT_TYPE_COUNTER2   0x14
#define INT_TYPE_COUNTER3   0x18
#define INT_TYPE_WD	    0x21
...
#define PCI_MAX_RESOURCES 6 // PCIe BARs 0-5
...
#define DE_THREAD_CANCEL            23  /* Windriver or Linux thread cancellation */

#define DE_INVALID_PARAMETER        34  /* Invalid parameter */
#define DE_HELIX_COMMUNICATION_BUSY	25	/*Helix communication is busy*/
...
#define STR_DE_HELIX_COMMUNICATION_BUSY "HELIX COMMUNICATION IS BUSY"
// used to specify max DIO ports for the board to determine what is the maximum
// across all boards - this happens to be fpgpio96
#define DSC_DIO_PORTS_MAX 8

// The maximum number of A/D reference voltages
#define AD_MAX_REFS_VOLT 20
...
typedef struct
{
    ...
    WORD pci_devid; // PCI device ID for the board
    WORD pci_vendorid ; // PCI vendor ID for the board.
    BYTE bytBAR ; // BAR to use for I/O
...
typedef struct
{
    ...
    BYTE clk;             /* INPUT: 0 = ADSTART=1, 1 = falling edge of P_ADTRIG, 2 = rising edge of conter0, 3 = rising edge of counter1 */
    BOOL gainTableE;      /* INPUT: TRUE = sets RTEN=1, FALSE = sets RTEN=0 RTEN tells FPGA to load gain table from internal memory */
    BOOL gateE;           /* INPUT: TRUE = set A/D gate enable, FALSE = disables A/D gate enable */
    BOOL turbo;           /* INPUT: TRUE = sets A/D turbo feature, FALSE = disables A/D turbo */
    BYTE low_channel;     /* low channel for A/D conversion */
    BYTE high_channel;    /* high channel for A/D conversion */
    BYTE prog_delay;      /* 8-bit programmable scan delay */
...
typedef struct
{
    ...
    BYTE   boot_adrange;        /* INPUT:  When called, this adrange is stored to BOOT values                 */
    FLOAT  ad_offset, ad_gain;  /* OUTPUT: Differences between target and measured values (FLOAT)             */
    DFLOAT target_values[AD_MAX_REFS_VOLT]; /* OUTPUT: Target values read from EEPROM                         */
    ...
    BYTE   calmux_output;       /* Bits[2-0] - channel selection, Bits[4-3] - calmux reference selection,
                                   Bit[5] - use all modes from table, Bit[6] - use all calmux output voltages
                                   Bit[7] - override bit set to high to use settings fro Bits[6-0]            */
    DFLOAT  offset, gain;       /* OUTPUT: Differences between target and measured values in volts            */
...
typedef struct
{
    WORD    daData ;            /* Data to write to the data registers of the DA chip*/
    BYTE    daCommand ;         /* DA Command to send to the DA chip*/
} DSCDAFUNCTION ;
...
{
	//// AMI for VEGA board
	//BYTE polarityA;		/* INPUT: 0 = BIPOLAR, 1 = UNIPOLAR */
	//BYTE polarityB;		/* INPUT: 0 = BIPOLAR, 1 = UNIPOLAR */

	//FLOAT rangeA;        /* INPUT: Absolute value of maximum voltage for 0-3 */
	//FLOAT rangeB;        /* INPUT: Absolute value of maximum voltage for 0-3 */

	//BYTE gainA;          /* INPUT: 0 = gain of 1, 1 = gain of 2, 2 = gain of 4, 3 = gain of 8 */
	//BYTE gainB;          /* INPUT: 0 = gain of 1, 1 = gain of 2, 2 = gain of 4, 3 = gain of 8 */

	// AMI
    BYTE polarity;      /* INPUT: 0 = BIPOLAR, 1 = UNIPOLAR */
    BYTE load_cal;      /* INPUT: 0 = FALSE, 1= TRUE. Loads calibration from eeprom */
//    FLOAT range;        /* INPUT: Absolute value of maximum voltage */
	BYTE range; // AMI : changed from float
    BYTE gain;          /* INPUT: 0 = gain of 1, 1 = gain of 2, 2 = gain of 4, 3 = gain of 8 */
    ...
    BYTE daB2C ;        /* 0 = Binary data, 1 = 2's complement data */
...
typedef struct
{
    ...
    BYTE control_code;      /* [INPUT] Counter command byte.- Refer Board manual for specifics */
    BYTE counter_number;    /* [INPUT] Counter number to which the settings are being applied. */
    DWORD counter_data;     /* [INPUT] Data to set the counter to */
    DSCCS counter0;         /* [OUTPUT] Counter 0 data read */
    DSCCS counter1;         /* [OUTPUT] Counter 1 data read */
    DSCCS counter2;         /* [OUTPUT] Counter 2 data read */
    BYTE control_bit;       /* [INPUT] Used in conjunction with control_code */
    DSCCS counter[DSC_DIO_PORTS_MAX]; /* [OUTPUT] Counter0-Max data read - used in fpgpio96 since there are 8 counters */
...
typedef struct
{
    ...
    BYTE pwm_command;    /* PWM Command to send to the pwm_circuit-refer to FPDAQ1616 manual*/
} DSCPWM;
...
typedef struct
{
	...
	BYTE cycle;          /* If cycle = 1, waveform restarts, otherwise runs only once - RMM-1616 only */
...
typedef struct
{
	...
	int	HF;						// 0 = number of A/D samples in FIFO is less than the programmed threshold, 1 = number of A/D samples in FIFO is equal to or greater than the programmed threshold
...
BYTE DSCUDAPICALL dscADSampleSleep(DSCB board, const DWORD ms);
...
BYTE DSCUDAPICALL dscADSetGainTable(DSCB board, BYTE* adcommand);
BYTE DSCUDAPICALL dscADIntStatus(DSCB board, DSCADINTSTATUS* intstatus);//added
BYTE DSCUDAPICALL dscFifoStatus(DSCB board, DSCFIFO* Fifo);//added
BYTE DSCUDAPICALL dscLEDTest(DSCB board,int enable);//added
/*/////////////////
// D/A Functions //
//////////////// */
...
BYTE DSCUDAPICALL dscDAFunction(DSCB board, WORD daData, BYTE daCommand);
BYTE DSCUDAPICALL dscDAUpdate(DSCB board);
...
BYTE DSCUDAPICALL dscDIOGetConfig(DSCB board, BYTE* config_bytes);
BYTE DSCUDAPICALL dscDIOSetPortConfig(DSCB board, BYTE port, BYTE config);
BYTE DSCUDAPICALL dscDIOGetPortConfig(DSCB board, BYTE port, BYTE* portConfig);
...
BYTE DSCUDAPICALL dscCounterFunction(DSCB board, DSCCR *dsccr);
...
BYTE DSCUDAPICALL dscDAMonitorSelect (DSCB board, BYTE daChannel );
...
BYTE DSCUDAPICALL dscPWMConfig(DSCB board, BYTE pwmChannel,short int pwmFreq,BYTE pwmDutyCycle);
BYTE DSCUDAPICALL dscPWMClear(DSCB board, BYTE pwm_circuit);
BYTE DSCUDAPICALL dscPWMFunction(DSCB board, DSCPWM * dscpwm);
BYTE DSCUDAPICALL dscPWMControl(DSCB board, BYTE pwm_control ) ;
...
BYTE DSCUDAPICALL dscCapabilitiesRead (DSCB board, BYTE* data);
BYTE DSCUDAPICALL dscFPCommand (DSCB board, BYTE cmd);
BYTE DSCUDAPICALL dscBoardReset (DSCB board, BYTE peripheral);
BYTE DSCUDAPICALL dscGetBoardConfig (DSCB board, BYTE* config);
BYTE DSCUDAPICALL dscCommand (DSCB board, BYTE cmdName, BYTE cmdValue);
BYTE DSCUDAPICALL dscInterruptConfig (DSCB board, BYTE config);

BYTE DSCUDAPICALL dscSetPage (DSCB board, BYTE page_number );
...
BYTE DSCUDAPICALL dscDIOCtrMode(DSCB board, BYTE mode);
BYTE DSCUDAPICALL dscPWMFnc(DSCB board, DSCPWM* dscpwm);
BYTE DSCUDAPICALL dscPWMCommand (DSCB board, DSCPWM* dscpwm );
