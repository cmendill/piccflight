...
void lyt_alp_zernpid(lytevent_t *lytevent, double *zernike_delta, int *zernike_switch, int reset){
  ...
  //Get sample index
  int sample = lytevent->hed.frame_number % LYT_NSAMPLES;

  //Run PID
  for(i=0;i<LOWFS_N_ZERNIKE;i++){
    if(zernike_switch[i]){
      //Calculate error
      error = lytevent->zernike_measured[i][sample] - lytevent->zernike_target[i];
      //Calculate integral
      zint[i] += error;
...
/**************************************************************/
void lyt_zernike_fit(lytevent_t *lytevent, int reset){
  FILE   *fd=NULL;
  char   filename[MAX_FILENAME];
  static int init=0;
  ...
  for(i=0;i<LYTXS;i++)
    for(j=0;j<LYTYS;j++)
      if(lyt_pxmask[i][j])
	total += (double)lytevent->image.data[i][j];
  for(i=0;i<LYTXS;i++)
    for(j=0;j<LYTYS;j++)
      if(lyt_pxmask[i][j])
	lyt_delta[count++]  = ((double)lytevent->image.data[i][j])/total - lyt_refimg[i][j];
  ...
  //Copy fit into event
  for(i=0;i<LOWFS_N_ZERNIKE;i++)
    lytevent->zernike_measured[i][sample] = zernikes[i];
}
...
void lyt_process_image(stImageBuff *buffer,sm_t *sm_p, uint32 frame_number){
  ...
  if(!init){
    ...
    for(i=0;i<LOWFS_N_ZERNIKE;i++){
      zernike_target[i] = sm_p->lyt_zernike_target[i];
      lytevent.zernike_target[i] = zernike_target[i];
    }
    //Set init flag
    init=1;
    //Debugging
  ...
  //Set sample index
  sample = lytevent.hed.frame_number % LYT_NSAMPLES;
  
  //Perform target operations on the first of each LYT_NSAMPLES
  if(sample == 0){
    //Save zernike targets
    for(i=0;i<LOWFS_N_ZERNIKE;i++){
      zernike_target[i] = sm_p->lyt_zernike_target[i];
	...
	sm_p->tgt_calmode = tgt_calibrate(lytevent.hed.tgt_calmode,zernike_target,&lytevent.hed.tgt_calstep,LYTID,FUNCTION_NO_RESET);
	for(i=0;i<LOWFS_N_ZERNIKE;i++){
  ...
  if(sm_p->state_array[state].lyt.fit_zernikes)
    lyt_zernike_fit(&lytevent, FUNCTION_NO_RESET);
  ...
  for(i=0;i<LOWFS_N_ZERNIKE;i++)
    lytevent.alp_zcmd[i][sample] = alp.zernike_cmd[i];
  ...
  //Write event to circular buffer on the last of each LYT_NSAMPLES
