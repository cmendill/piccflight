...
etStat BOBCAT_ControlRead(tHandle hCamera, ui8** msgBuffer, ui8* msgLength) {
  ...
  if ( msgLength != 0 ) {
    ...
    *msgBuffer = (ui8*)malloc((*msgLength)*sizeof(ui8));
...
etStat BOBCAT_ParameterGet(tHandle hCamera, bobcatParam parameter, void* value) {
  ui8 *rxMsgBuffer, *txMsgBuffer;
  ui8 rxMsgLength, txMsgLength;
  etStat eStat = PHX_OK;

  txMsgLength = 3;

  rxMsgBuffer = NULL;
  txMsgBuffer = (ui8*)malloc(txMsgLength*sizeof(ui8));
  ...
  Return:
  free( txMsgBuffer ); /* malloced here */
  free( rxMsgBuffer ); /* malloced in BOBCAT_ControlRead() */
  return eStat;
}
