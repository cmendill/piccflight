...
static void phxshwfs_callback( tHandle hCamera, ui32 dwInterruptMask, void *pvParams ) {
...
int phxshwfs( etParamValue eBoardNumber, char *configFileName ) {
  ...
  eStat = PHX_StreamRead( hCamera, PHX_START, (void*)phxshwfs_callback );
  if ( PHX_OK != eStat ) goto Error;
